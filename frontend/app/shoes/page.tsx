...
export default function ShoesPage() {
  ...
  const shoeProductsFromDB = useMemo(() => {
    if (!products) return [];
    return products.filter((p: any) => p.category === "Shoes" || p.sub_category === "Shoes");
  }, [products]);
          ...
          {visibleProducts.map((product: any) => (
            <div key={product.id} className="group">
              ...
              <div className="flex justify-between items-end px-4 mb-2">
                <h3 className="font-black text-slate-900 text-lg uppercase tracking-tighter group-hover:text-blue-600 transition-colors line-clamp-1 flex-1 pr-2">
                  {product.name}
                </h3>
                <div className="text-right shrink-0">
                  {product.is_on_sale && product.sale_percentage > 0 ? (
                    <div className="flex flex-col">
                      ...
                      <span className="font-black text-red-600 text-xl italic">
